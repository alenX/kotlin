 ...
 */
fun getCorrespondingCall(expression: JetExpression?, bindingContext: BindingContext): Call? {
    val expr = JetPsiUtil.deparenthesize(expression)
    if (expr == null) return null

    if (expr is JetQualifiedExpression) {
        return getCorrespondingCall(expr.getSelectorExpression(), bindingContext)
    }
...
fun getEnclosingCall(expression: JetExpression?, bindingContext: BindingContext): Call? {
    val parent = PsiTreeUtil.getParentOfType<JetExpression>(
            expression,
    ...
    return getCorrespondingCall(parent, bindingContext)
}
