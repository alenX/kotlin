...
class ResolutionResultsCacheImpl : ResolutionResultsCache {
    ...
    override fun record(
        ...
        data.put(call, CachedData(results, deferredComputation, tracing, resolutionTrace))
    }
...
public class TemporaryResolutionResultsCache(private val parentCache: ResolutionResultsCache) : ResolutionResultsCache {
    ...
    override fun record(call: Call, results: OverloadResolutionResultsImpl<out CallableDescriptor?>, deferredComputation: BasicCallResolutionContext, tracing: TracingStrategy, resolutionTrace: DelegatingBindingTrace) {
    ...
    public fun commit() {
        if (parentCache is ResolutionResultsCacheImpl) {
            parentCache.addData(innerCache)
        }
        else if (parentCache is TemporaryResolutionResultsCache) {
            parentCache.innerCache.addData(innerCache)
        }
    }
}