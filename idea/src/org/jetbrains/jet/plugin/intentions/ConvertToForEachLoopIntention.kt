...
public class ConvertToForEachLoopIntention : JetSelfTargetingIntention<JetExpression>("convert.to.for.each.loop.intention", javaClass()) {
    ...
    override fun isApplicableTo(element: JetExpression): Boolean {
        fun isWellFormedFunctionLiteral(element: JetFunctionLiteralExpression): Boolean {
            return element.getValueParameters().size() <= 1 &&
        ...
        return callee is JetSimpleNameExpression &&
        callee.getReferencedName().equals("forEach") &&
    ...
    override fun applyTo(element: JetExpression, editor: Editor) {
        ...
        element.replace(JetPsiFactory.createExpression(element.getProject(),
                                                       if (functionLiteral.getValueParameters().size() == 0) {
                                                           "for (it in ${receiver!!.getText()}) { ${functionLiteral.getBodyExpression()!!.getText()} }"
                                                       } else {
                                                           "for (${functionLiteral.getValueParameters()[0].getText()} in ${receiver!!.getText()}) { ${functionLiteral.getBodyExpression()!!.getText()} }"
                                                       }))
